...
from src.feat.labels_risk_adj import LABEL_Z_WINDOW, build_label_for_sample


def _limit_df(dates: list[str], limit_pct: float = 0.1) -> pd.DataFrame:
    return pd.DataFrame({
        "trade_date": dates,
        "up_limit": np.full(len(dates), 110.0),
        "down_limit": np.full(len(dates), 90.0),
        "limit_pct": np.full(len(dates), limit_pct),
    })
...
def test_label_zscore_strict_full_window() -> None:
    ...
    bad_result = build_label_for_sample("AAA", asof_insufficient, calendar_str, lambda _: df, limit_loader=lambda _: limit_df)
    assert not bad_result.label_ok
    assert "label zscore" in (bad_result.fail_reason or "")

    asof_ok = calendar[282].isoformat()
    ok_result = build_label_for_sample("AAA", asof_ok, calendar_str, lambda _: df, limit_loader=lambda _: limit_df)
    assert ok_result.label_ok
    assert np.isfinite(ok_result.y_z)
    assert abs(float(ok_result.y_z)) <= 1.0 + 1e-4
    ...
    zero_sd_result = build_label_for_sample("AAA", asof_ok, calendar_str, lambda _: flat_df, limit_loader=lambda _: limit_df)
