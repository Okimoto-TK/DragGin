...
## LocalProvider usage (non-pytest manual test)
`LocalProvider` is available in `src/feat/build_multiscale_tensor.py` and can read CSV/Parquet for local runs.

Example command:

```bash
python tools/manual_local_test.py \
  --code 000001.SZ \
  --asof 2026-02-13 \
  --calendar-dir /path/to/daily_calendar_files \
  --root-1d /path/to/bars/1d \
  --root-30m /path/to/bars/30m \
  --root-5m /path/to/bars/5m
```

LocalProvider default file naming (per interval root):
- `<code>.csv` / `<code>.parquet`
- `<code>_<interval>.csv` / `<code>_<interval>.parquet`
